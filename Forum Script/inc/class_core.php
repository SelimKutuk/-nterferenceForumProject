...
class MyBB {
	...
	function trigger_generic_error($code)
	{
		...
		{
			case "cache_no_write":
				$message = "FTP (./cache) klasörünün yazılabilir olması gerekiyor. Lütfen (./cache) klasörünün CHMOD izinlerini (777) yapınız.";
				$error_code = MYBB_CACHE_NO_WRITE;
				break;
			case "install_directory":
				$message = "(./install) klasörünün hala ftp/sunucunuzda yüklü olduğu tespit edildi ve kilitli değil. Lütfen (./install) klasörünü ya silin yada adını değiştirin. Alternatif olarak (./install) klasörünün içine '.lock' adında boş bir dosya oluşturup kilitlenmesinide sağlayabilirsiniz.<br /><br /><b>Not:</b> Bu hata eğer güncelleme dosyalarını yükledikten hemen sonra oluştu ise, Bu durumda adres satırına <b>site-adınız.com/install/upgrade.php</b> şeklinde yazarak güncelleme sihibazını çalıştırın ve direktifleri takip ederek versiyon yükseltme işlemini tamamlayınız.";
				$error_code = MYBB_INSTALL_DIR_EXISTS;
				break;
			case "board_not_installed":
				$message = "MyBB forum kurulumunu henüz gerçekleştirmemişsiniz, bu hatayı almanız gayet normal. Lütfen adres satırına <b>site-adınız.com/install</b> şeklinde yazarak, kurulum işlemini başlatınız.";
				$error_code = MYBB_NOT_INSTALLED;
				break;
			case "board_not_upgraded":
				$message = "MyBB forum versiyonunuzun henüz yükseltilmediği algılandı. Lütfen bunun doğruluğunu kontrol ediniz.";
				$error_code = MYBB_NOT_UPGRADED;
				break;
			case "sql_load_error":
				$message = "MyBB, SQL veritabanı uzantısını yükleyemedi. Lütfen Bu hatanın çözümü için: <a href=\"http://destek.mybb.com.tr\" target=\"_blank\"><b>MyBB Türkiye Resmi Destek</b></a> forumunu ziyaret ediniz.";
				$error_code = MYBB_SQL_LOAD_ERROR;
				break;
			case "eaccelerator_load_error":
				$message = "eAccelerator önbellek desteğini kullanabilmeniz için sunucunuzdaki PHP derlemesinin yeniden yapıladırılması gerekir. Bu konu hakkında bilginiz yok ise, lütfen hosting sağlayıcınızdan yardım isteyiniz.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "memcache_load_error":
				$message = "Sunucunuzda <b>memcache</b> desteğinin etkin olmadığı tespit edildi. Bu konu hakkında bilginiz yok ise, lütfen hosting sağlayıcınızdan yardım isteyiniz.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "xcache_load_error":
				$message = "XCache PHP ile XCache Önbellek Desteğini kullanabilmeniz için ./inc/config.php dosyasında yapılandırılmış olması gerekiyor. Bu konu hakkında bilginiz yok ise, lütfen <a href=\"http://destek.mybb.com.tr\" target=\"_blank\"><b>MyBB Türkiye Resmi Destek</b></a> forumunu ziyaret ediniz.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			default:
				$message = "MyBB, dahili bir hata olduğunu tespit etti ve devam edemiyor. Lütfen Bu hatanın çözümü için: <a href=\"http://destek.mybb.com.tr\" target=\"_blank\"><b>MyBB Türkiye Resmi Destek</b></a> forumunu ziyaret ediniz.";
