...
class errorHandler {
	...
	public $error_types = array( 
		...
		E_USER_DEPRECATED	 => 'Onaylanmamış Kullanıcı Uyarısı',
		E_STRICT             => 'Bildirim Notu',
		E_RECOVERABLE_ERROR  => 'Onarılabilir Önemli Hata',
		...
		MYBB_CACHEHANDLER_LOAD_ERROR => 'MyBB Hatası',
	);
	...
	function error($type, $message, $file=null, $line=0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					echo "<div class=\"php_warning\">MyBB Dahili: Bir veya daha fazla uyarı oluştu. Lütfen yardım için sistem yöneticinize başvurun.</div>"; 
				}
				else
				{
					global $templates;
	...
	function email_error($type, $message, $file, $line)
	{
		...
		$message = "{$mybb->settings['bbname']} ({$mybb->settings['bburl']}) , Sitenizde çalışan şu anki mybb sürümünüzde aşağıdaki hatalar oluştu. Hata ayrıntıları:\n---\nHata Tipi: $type\nDosya: $file (Satır No: $line)\nHata Mesajı\n$message";

		@my_mail($mybb->settings['adminemail'], "{$mybb->settings['bbname']} Sitenizde MyBB Hatası Oluştu", $message, $mybb->settings['adminemail']);
	}
	...
	function output_error($type, $message, $file, $line)
	{
		...
		{		
			...
			$error_message = "<p>MyBB, bir SQL hatası olduğunu tespit etti ve aşağıdaki nedenlerden dolayı devam edemiyor.</p>";
		...
		else
		{
			...
			if($mybb->settings['errortypemedium'] == "both" || $mybb->settings['errortypemedium'] == "error" || defined("IN_INSTALL") || defined("IN_UPGRADE"))
			{
				...
				{
					$error_message .= "<dt>Geri Bildirim Raporu:</dt><dd>{$backtrace}</dd>\n";
				}
				$error_message .= "</dl>\n";
			}
		}
