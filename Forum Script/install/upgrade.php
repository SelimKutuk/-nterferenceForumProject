...
$output->title = "MyBB Versiyon Yükseltme Sihirbazı";
...
else
{
	...
	{	
		...
		{
			$output->print_error("Kullanıcı kimliğiniz çıkış yapabilmeniz için doğrulanamadı. Kötü niyetli bir Javascript çıkış yapabilmeniz için sizi engelliyor olabilir. Çıkış yapabilmek için lütfen üst menüdeki <b>çıkış yap</b> linkine tıklayınız.");
		}
	...
	{	
		...
		{
			$output->print_error("Girmiş olduğunuz kullanıcı adı gerçersiz.");
		}
		...
		else
		{
			...
			if(!$user['uid'])
			{
				$output->print_error("Girmiş olduğunuz şifre geçersiz. Eğer şifrenizi unuttuysanız, <a rel=\"nofollow\" href=\"../member.php?action=lostpw\">Buraya</a> tıklayıp şifrenizi yenileyiniz. Daha sonra geri dönüp tekrar devam etmeyi deneyiniz.");
			}
		}
...
<form action="upgrade.php" method="post">
	...
	elseif($mybb->input['action'] == "doupgrade")
	{
		...
		if($db->table_exists("datacache") && $upgrade_detail['requires_deactivated_plugins'] == 1 && $mybb->input['donewarning'] != "true")
		{
			...
			{
				$output->print_header();
				$lang->plugin_warning = "<input type=\"hidden\" name=\"from\" value=\"".intval($mybb->input['from'])."\" />\n<input type=\"hidden\" name=\"donewarning\" value=\"true\" />\n<div class=\"error\"><strong><span style=\"color: red\">Uyarı:</span></strong> <p>Tespit edilen aktif eklentiler: ".count($plugins['active'])." Lütfen güncellleme yapmadan önce forumunuzda yüklü tüm eklentileri devre dışı bırakınız. Aksi halde sorunlar ile karşılaşabilirsiniz.</p></div> <br />";
...
function upgradethemes()
{
	...
	if($system_upgrade_detail['revert_all_themes'] > 0)
	{
		...
		{
			$output->print_error("Lütfen ./ADMİN klasörünün doğru bir şekilde yüklendiğinden emin olun.");
		}
...
function buildsettings()
{
	...
	{
		$output->print_header("Ayar Kontrolü");
		echo "<p><div class=\"error\"><span style=\"color: red; font-weight: bold;\">Hata: ./inc/settings.php dosyasına veriler yazılamıyor.</span><h3>Lütfen güncelleme işlemine devam edebilmek için: ./inc/settings.php dosyasının CHMOD izinlerini (777) yapınız.</h3></div></p>";
		$output->print_footer("rebuildsettings");
		exit;
	}
...
function upgradedone()
{
	...
	ob_start();
	$output->print_header("Güncelleme İşlemi Tamamlandı");
